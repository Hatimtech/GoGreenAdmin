...
<div class="right_col" id="cool" role="main">
      ...
      <div class="x_panel">
        ...
        <div class="x_content">
          <form method="post" action="<?php echo base_url('push_notification/send')?>" id="pushForm">
            <input type="hidden" name="for" value="cleaners">
 ...
 "type_id" : "10"
            ...
            <div class="form-group">
              <label class="text-danger">Select one or more cleaners or leave it blank to send push notification to all cleaners</label>
              <select name="users[]" theme="google"  placeholder="Select Your Favorite" id="users" data-search="true"  class="form-control" multiple>
                ...
                <?php
                  if(!empty($users))
                  {
                    ...
                    {
                      echo"<option value='".$value['device_type']."^".$value['device_token']."'>".$value['first_name']." ".$value['last_name']." (".$value['email'].")</option>";
                    }
                  }
                ?>
              </select>
