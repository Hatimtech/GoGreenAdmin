...
class Api_v2 extends MY_Controller
{
  ...
  public function test($array){
    ...
    public function phone_varification($request = null)
    {
    	if($request->user_id)
    	{
    		$user = $this->db->where('id', $request->user_id)->get('users')->row();
            $otp = mt_rand(1111, 9999);
            $data = $this->db->insert('opt_varification', ['user_id'=>$user->id, 'otp'=>$otp]);
           // print_r($data); die;
            if($data)
            {
                ...
                else
                {
                    $Code = ResponseConstant::UNSUCCESS;
                    $rescode = ResponseConstant::UNSUCCESS;
                    $Message = "Error! please check your phone number";
                    $this->sendResponse($Code,$rescode, $Message);
                }
                die;
            }
    ...
    public function update_verification_key($request = null)
    {
    	...
    	{
            $data = $this->db->where('user_id', $request->user_id)
            			->where('verified', 0)
            			->where('attempt <=', 5)
            ...
            if($data)
            {
            	...
            	$this->db->where('id', $data->id)->update('opt_varification', ['attempt'=> ++$data->attempt, 'verified' => $verified]);
            	if($verified)
            	{
            		$Code = ResponseConstant::SUCCESS;
	                $rescode = ResponseConstant::SUCCESS;
