...
class Additional_coupons extends MX_Controller {
	public $ftres;
	function __construct()
    {
		parent::__construct();
		 $this->load->library('form_validation');
		$this->load->model('additional_coupons_model');
	...
	{
     $coupans = $this->additional_coupons_model->get_all_coupans($this->ftres);
	...
	public function add_coupans()
	{
		...
		if($_POST)
		{
			...
			{
				$bool = $this->additional_coupons_model->check_img($_FILES['coupan_image']['name']);
				if($flag==1)
				{
					$bool==0;
				}
				...
				else
				{
					...
					if ($this->form_validation->run() == true)
					{
						...
						$additional_services_id = $this->input->post('additional_services_id');
						...
						else
						{
							...
							//echo $filename;die;
							 ...
							 // print_r($data);die;
							 $bool = $this->additional_coupons_model->insert_coupan($data,$flag,$coupan_id);
							 if($bool)
							 {
							 	$this->session->set_flashdata('succ','Coupons Added Successfully');
							 	redirect('additional_coupons');
							 }
							 else
							 {
		...
		$additional = $this->additional_coupons_model->get_all_additional_services();
		if($flag==1)
		{
			// $coupan_id = $this->input->get('id');
			$row = $this->additional_coupons_model->get_coupans_for_edit($coupan_id);
			// print_r($row); die;
			$data['coupan'] = $row;
	...
	public function check_coupan_exis()
	{
		...
		$row = $this->additional_coupons_model->check_coupan_code_exist($coupan_code);
		if($row)
		{
	...
	{
		$coupan_id = $this->input->get('id');
		$bool = $this->additional_coupons_model->delete_coupan_by_id($coupan_id);
		if($bool)
		{
